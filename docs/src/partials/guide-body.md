...
- Context payloads are wrapped in XML-style tags (`<release_context>`, `<diff>`, `<commit_log>`, `<existing_pr>`) so the model can disambiguate sections.
...
- `_git.py`: git helpers (`test_git_utils.py`)
