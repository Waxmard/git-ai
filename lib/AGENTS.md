...
Gemini request bodies and response extraction are shared between the AI Studio and Vertex paths (`_gemini_request_body`, `_extract_gemini_text`). The extractor joins every non-`thought` text part: thinking models return reasoning as flagged parts, so `parts[0]` is not reliably the answer.
...
- A section suffix is an *address*, not necessarily the GCP project: a hand-written `project =` can point `[vertex-anthropic@acme]` at `acme-prod`. `_setup_vertex_resolved_project` resolves that (checking both family sections, since either can carry the override) and `_setup_vertex_add_project` consults it before writing, so re-picking `acme-prod` recognises the existing alias instead of creating a second profile. The summary renders the divergence as `acme (project: acme-prod)`. Fast-path reset snapshots and restores the override, or it would silently repoint the profile at its own suffix.
