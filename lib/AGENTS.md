...
- `_setup_vertex_normalize` folds the legacy shared shape down to per-project sections, copying each base section's models into every project and then dropping the base sections and the `project=`/`projects=` keys that drove the expansion; a leftover base `project =` would override every profile's own project. It runs **only from the three write paths**, so an edit the user backs out of leaves the file untouched, and each of them aborts if it refuses — writing over a config still in the shared shape leaves base models expanding, which is what makes a scoped unpin silently do nothing.
- The fold is **all-or-nothing**: `_setup_vertex_normalize` is a wrapper that backs the file up, runs `_setup_vertex_fold`, and restores if any pinned model id went missing. The fold only ever *moves* pins, so a dropped id means a section was removed before its models were copied out — silent, unrecoverable loss otherwise. Each base section carries its own `project =` and they need not agree, so the project list is the **union** of them (`_setup_current_vertex_projects`); stopping at the first is what strands the other's models and trips the guard.
- A base `project =` stops applying to profiles once it's dropped — that is the point of per-project pinning, since a base key retargets every profile at once. But it does move where an inheriting profile runs, so `_setup_vertex_inherited_projects` names each one on stderr with the `project =` line to add back. Only a profile's **own** `project =` is an alias; `_setup_vertex_alias_map` deliberately ignores inherited values when deciding where to fold a project's pins.
- Folding a project onto the alias that addresses it means its pins have **two** keys in `parse_user_options`: base-section expansion is keyed under the project id the expansion used, never the alias. `_setup_vertex_sources` reads both, or the inherited pins vanish from the aliased project — and the loss guard misses it whenever the same id is still pinned somewhere else.
- An override lives in **one family's** section, but `vertex_resolve` never consults the sibling, so a wizard write that creates the other family's section for the same suffix would have gemini call `acme` while anthropic calls `acme-prod`. `_setup_vertex_mirror_alias` copies it across — only onto sections just created, since retargeting one the user already had is the silent repoint above.
