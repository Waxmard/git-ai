...
Every PR prompt receives `<diff>`, including the conventional-commit two-pass path. The draft supplements the patch; commit messages and file stats alone cannot reveal incompatible interface changes. Compatibility warnings remain independent of the title type, and `!` is left to the owner.
